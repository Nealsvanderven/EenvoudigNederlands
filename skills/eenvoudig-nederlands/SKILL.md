...
18. **Waarschuwing: eerst de opdracht of de voorwaarde, dan het risico.** "Voer dit commando niet uit op productie. Het commando verwijdert rijen."
19. **Getallen en datums.** Gebruik cijfers voor meetbare waarden, met een komma als decimaalteken. Schrijf een datum voluit: 11 september 2026. Gebruik de ISO-notatie alleen in logregels en code.
...
3. De eerste zin geeft het antwoord of de uitkomst. Herhaal de vraag niet.
4. Geen gedachtestreepje. Noem het verband ("want", "maar", "bijvoorbeeld") of schrijf twee zinnen.
5. Leg een vakterm bij het eerste gebruik in een paar woorden uit: "idempotent (je kunt het veilig twee keer draaien)". Leg productnamen niet uit.
6. Geen spreektaal en geen samentrekkingen ('t, z'n, d'r, ie). Geen aanloopjes ("Zeker", "Goede vraag") en geen afsluiters ("Ik hoop dat dit helpt", "Laat het me weten").
7. Kort geciteerde foutmeldingen, veiligheidswaarschuwingen en bevestigingen voor een onomkeerbare actie nooit in.
...
1. Antwoord: tel de zinnen. Meer dan vijf, dan schrappen. Zoek naar `—`, `**`, `#` en een regel die met `-` begint. Haal elke treffer weg.
...
**B1** is de standaard en bestaat uit alles hierboven. **Strikt** geldt als de gebruiker B1, 2F, Direct Duidelijk of een taalniveautoets noemt: lees dan `references/taalniveaus.md` voordat je het document schrijft. Zeg één keer dat geen enkel hulpmiddel taalniveau B1 garandeert en dat een menselijke toets of een meetinstrument het laatste woord heeft. Het antwoord in de chat blijft in elke modus hetzelfde.
